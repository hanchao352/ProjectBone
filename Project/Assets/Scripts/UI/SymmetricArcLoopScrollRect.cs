...
public class SymmetricArcLoopScrollRect : MonoBehaviour
{
    ...
    private void UpdateItems()
    {
        ...
        for (int i = firstItemIndex; i < firstItemIndex + itemsInView; i++)
        {
            ...
            float curveOffset = 2 * (itemTransform.anchoredPosition.y + scrollPos) / scrollRect.viewport.rect.height - 1;
            float curvedX = curveFactor * curveOffset * curveOffset;

            if (curveOffset > 0)
                item.transform.localPosition = new Vector3(-curvedX, item.transform.localPosition.y, item.transform.localPosition.z);
            else
                item.transform.localPosition = new Vector3(curvedX, item.transform.localPosition.y, item.transform.localPosition.z);
